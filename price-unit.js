(function(root,factory){
  ...
  const stopWords=new Set(['de','del','la','el','las','los','con','c','por','x','unidad','unidades','individual','un','u']);
  ...
  function searchTokens(value){
    return searchText(value).replace(/\bbon\s*o\s*bon\b/g,'bonobon')
      ...
      .filter(Boolean).map(token=>aliases[token]||token).filter(token=>!stopWords.has(token));
  }
  ...
  return {isIndividual,isKioskProduct,matchesSearch};
});
