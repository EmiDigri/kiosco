...
function dayWindow(date) {
  ...
  const dayEnd = new Date(Date.UTC(year, month - 1, day + 1, 2, 59, 59));
...
function reportOutflows(csv) {
  ...
  const dateIndex = indexOf('TRANSACTION_DATE', 'SETTLEMENT_DATE', 'FECHA DE ORIGEN');
  ...
  if (dateIndex < 0 || netIndex < 0) return [];
  ...
  rows.slice(1).forEach((values, index) => {
    ...
    const sourceId = values[idIndex] || `report-${date}-${index}`;
    unique.set(String(sourceId), {
      ...
      status: 'approved',
      es_enviada: true,
      description: values[typeIndex] || 'Transferencia enviada',
    });
  });
...
export async function settlementOutflows(date, token) {
  ...
  const reports = listResponse.ok ? await listResponse.json().catch(() => []) : [];
  const matching = (Array.isArray(reports) ? reports : [])
  ...
  if (processed) {
    ...
    return { status: 'processed', outflows: reportOutflows(await download.text()) };
  }
