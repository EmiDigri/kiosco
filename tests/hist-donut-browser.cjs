...
let histRowsPagosMes=[['Coca-Cola FEMSA',553976],['Edenor',481408],['Transferencia enviada',272327],['Producto',63460],['MAPFRE Aconcagua',54589],['Barracas Logistica',34875],['Lector',39539]].map((x,i)=>({fecha:'2026-09-'+String(18-i).padStart(2,'0'),nombre:x[0],monto:x[1],es_enviada:true,status:'approved'}));
${renderSource}
histRenderGastosMes();
</script></body></html>`;
...
(async()=>{
  ...
  try{
    ...
    check(await page.locator('.hist-egr-grp').count()===9,'no oculta conceptos de la lista');
